...
env_alloc(struct Env **newenv_store, envid_t parent_id)
{
	...
	*newenv_store = e;
	...
	// Empieza el codigo de autoria del grupo.

	// Inicializamos las ejecuciones en cero.
	e->ejecuciones = CERO;

	// Asignamos un puntero al proceso padre.
	struct Env *padre = NULL;
	envid2env(parent_id, &padre, CERO);

	// Insertamos el proceso en la cola correspondiente del scheduler.
	sched_agregar_env(e->env_id, padre ? padre->cola_actual : CERO);

	// Termina el codigo de autoria del grupo.
