...
// PRE: El "env_id" debe corresponder a un proceso valido.
// POST: Devuelve la prioridad del proceso con el "env_id" asociado.
static int
sys_env_obtener_prioridad(envid_t env_id)
{
	struct Env *env;
	int prioridad_actual;
	if ((prioridad_actual = envid2env(env_id, &env, CERO))) {
		return prioridad_actual;
	}

	return env->cola_actual;
}

// PRE: El "env_id" debe corresponder a un proceso valido.
// POST: La prioridad del proceso con el "env_id" asociado se actualiza con la recibida por parametro.
static int
sys_env_asignar_prioridad(envid_t env_id, int prioridad)
{
	if (prioridad < CERO || prioridad > TRES) {
		return MENOS_UNO;
	}
	...
	sched_eliminar_env(env->env_id);
	sched_agregar_env(env->env_id, prioridad);
	env->cola_actual = prioridad;
	return prioridad;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_yield:
		sys_yield();         // No return
	case SYS_obtener_prioridad:  // Syscall de autoria propia.
		return sys_env_obtener_prioridad(a1);
	case SYS_asignar_prioridad:  // Syscall de autoria propia.
		return sys_env_asignar_prioridad(a1, a2);
	default:
		return -E_INVAL;
	}
}
