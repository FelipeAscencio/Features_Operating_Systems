...
#define TESTED(x) (__builtin_strcmp(#x, STRING(TEST)) == 0)
	...
	if (TESTED(user_yield) || TESTED(user_spin0))
		ENV_CREATE(TEST, ENV_TYPE_USER);
#else
	// Los 3 test de autoria propia del grupo.
	ENV_CREATE(user_obtener_prioridad, ENV_TYPE_USER);
	ENV_CREATE(user_actualizar_prioridad, ENV_TYPE_USER);
	ENV_CREATE(user_asignacion_invalida, ENV_TYPE_USER);
#endif  // TEST*
