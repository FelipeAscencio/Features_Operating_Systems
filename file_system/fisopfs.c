...
fisopfs_getattr(const char *path, struct stat *st)
{
	...
	char *pos;
	if ((pos = strchr(path_copy, '\n')) != NULL) {
		*pos = '\0';
	}

	int inode_index = search_inode(path_copy);
              ...
              struct fuse_file_info *fi)
{
	...
	if (offset + size > file->size) {
		...
		// Inicializar la memoria nueva a ceros si hay "huecos" en el archivo
		if (offset > file->size) {
			memset(file->content + file->size, 0, offset - file->size);
		}
	}
...
fisopfs_truncate(const char *path, off_t size)
{
	...
	inode->content = realloc(inode->content, size * sizeof(char));
...
fisopfs_init(struct fuse_conn_info *conn)
{
	...
	int fp = open(DEFAULT_FILE_DISK, O_RDONLY);

	if (fp < 0) {
		create_inode("/", __S_IFDIR | 0755, INODE_DIR);
		fp = open(DEFAULT_FILE_DISK, O_WRONLY | O_CREAT, 0644);
...
fisopfs_destroy(void *userdata)
{
	...
	int fp = open(DEFAULT_FILE_DISK, O_WRONLY | O_TRUNC, 0644);
